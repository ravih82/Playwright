
// Export variables directly for easy usage
const URL = 'https://e1-poc-sandbox.a99d04.metricstream.com/metricstream/auth/signin.jsp';
const OwnerOrg = 'ACME Corp';
const Perspective = 'Operational Risk (Scoring Algorithm)';
const PurposeScope = 'Purpose scope entered';
const Frequency = 'Specific Date';
const NextReviewDate = '08/25/2026';
const Process = 'ABFL - Policy Governance - CSB';
const Risk = 'Strategic Risk';
const Assessor = 'ERM Program Manager';
const Approver = 'ORM Risk Manager';
const DueDate = '08/30/2026';
const FinancialRating = 'Low';
const ReputationalRating = 'Low';
const Likelihood = 'Likely';
const ApproverComments = 'Approved Risk Assessment';

module.exports = {
    URL: URL,
    OwnerOrg: OwnerOrg,
    Perspective: Perspective,
    PurposeScope: PurposeScope,
    Frequency: Frequency,
    NextReviewDate: NextReviewDate,
    Process: Process,
    Risk: Risk,
    Assessor: Assessor,
    Approver: Approver,
    DueDate: DueDate,
    FinancialRating: FinancialRating,
    ReputationalRating: ReputationalRating,
    Likelihood: Likelihood,
    ApproverComments: ApproverComments
};
